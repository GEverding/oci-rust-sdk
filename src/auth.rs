...
/// OCI CLI-based authentication that uses instance principals
/// This delegates to the OCI CLI which properly handles instance principal authentication
pub struct OciCliAuth {
    pub region: String,
    pub cli_path: String,
}
...
impl OciCliAuth {
    pub fn new(region: Option<String>) -> Self {
        ...
        Self {
            region,
            cli_path: "oci".to_string(), // Assumes oci CLI is in PATH
        }
    }
    ...
    /// Get region from OCI CLI config or use default
    async fn get_cli_region(&self) -> Result<String, AuthError> {
        // Try to get region from CLI config, fall back to provided region
        match tokio::process::Command::new(&self.cli_path)
            .args(&["setup", "config"])
            .output()
            .await
        {
            Ok(_) => Ok(self.region.clone()),
            Err(_) => Ok(self.region.clone()),
        }
    }
}

#[async_trait]
impl AuthProvider for OciCliAuth {
    async fn sign_request(
        &self,
        ...
        // OCI CLI authentication doesn't use manual request signing
        // The CLI handles authentication internally
        Ok(())
    }

    async fn get_tenancy_id(&self) -> Result<String, AuthError> {
        // Try to get tenancy from CLI config
        let output = tokio::process::Command::new(&self.cli_path)
            .args(&["iam", "tenancy", "get"])
            .arg("--raw-output")
            .output()
            .await
            .map_err(|e| AuthError::MetadataError(format!("Failed to execute OCI CLI: {}", e)))?;

        if !output.status.success() {
            return Err(AuthError::MetadataError(
                "Failed to get tenancy from OCI CLI".to_string(),
            ));
        }

        let output_str = String::from_utf8_lossy(&output.stdout);
        // Parse JSON response to get tenancy ID
        ...
        parsed["data"]["id"]
            .as_str()
            .map(|s| s.to_string())
            .ok_or_else(|| AuthError::MetadataError("Tenancy ID not found in CLI response".to_string()))
    }

    async fn get_region(&self) -> Result<String, AuthError> {
        self.get_cli_region().await
    }
}
